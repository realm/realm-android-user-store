...
try {
  node('android') {
    ...
    ws('/tmp/realm-android-user-store') {
      stage('SCM') {
        checkout([
          $class: 'GitSCM',
...
def collectAarMetrics() {
  sh """set -xe
    ...
    unzip secure-userstore-release.aar -d unzipped
  ...
  def methods = readFile('app/build/outputs/aar/methods')
  sendMetrics('secure_userstore_methods', methods, [:])

  def aarFile = findFiles(glob: 'app/build/outputs/aar/secure-userstore-release.aar')[0]
  sendMetrics('secure_userstore_aar_size', aarFile.length as String, [:])
}
